...
window.AudioContext = window.AudioContext || window.webkitAudioContext || noWebAudio();
...
const freqArray = new Array (185, 207.65, 233.08, 277.18, 311.13, 369.99, 415.30, 466.16, 196, 220, 246.94, 261.63, 293.66, 329.63, 349.23, 392, 440);
var currentOctave = 1;
...
window.onkeydown = function (e) {
	...
	if((x==16)||(x==17)||(x==18)||(x==91)){keyBugFix++;}
...
function stopMouseNote(note,event){
	...
	let woop = false;
	if((event==true)||(event.button==0)){woop=true;}
...
function Note(note,octave,type,id){
	...
	if(type=='key'){this.x = keyArray;}
		else if(type=='mouse'){this.x = mouseArray;}
		...
		if(type!='sequencer'){this.x[note] = thisOne;}
			allNotes.push(null);

		if(type=='sequencer'){this.type = 'sequencer';}
	...
	if(this.osc1Coarse<0){this.osc1Coarse = this.osc1Coarse/2;}
	this.osc2Coarse = document.getElementById('osc2Coarse').value/50;
	if(this.osc2Coarse<0){this.osc2Coarse = this.osc2Coarse/2;}
	...
	if(this.glide==true){this.osc1.frequency.setTargetAtTime(this.newNoteFrequency1, context.currentTime, this.glideAmount)};
	...
	if(this.glide==true){this.pulse1.frequency.setTargetAtTime(this.newNoteFrequency1, context.currentTime, this.glideAmount)};
	...
	if(this.glide==true){this.osc1FM.frequency.setTargetAtTime(this.newNoteFrequency2/2, context.currentTime, this.glideAmount)};
	...
	if(this.glide==true){this.osc2.frequency.setTargetAtTime(this.newNoteFrequency2, context.currentTime, this.glideAmount)};
	...
	if(this.glide==true){this.pulse2.frequency.setTargetAtTime(this.newNoteFrequency2, context.currentTime, this.glideAmount)};
	...
	if(this.glide==true){this.osc2FM.frequency.setTargetAtTime(this.newNoteFrequency1/2, context.currentTime, this.glideAmount)};
	...
	whiteNoise.connect(this.whiteNoiseAmpMod);
	...
	if(this.glide==true){this.whiteAmpModulator.frequency.setTargetAtTime(this.newNoteFrequency, context.currentTime, this.glideAmount)};
	this.whiteAmpModAmount = context.createGain();
	this.whiteAmpModAmount.gain.value = document.getElementById('noiseAmpMod').value/100;
	...
	if(this.glide==true){this.sub.frequency.setTargetAtTime(this.newNoteFrequency/2, context.currentTime, this.glideAmount)};
	...
	this.filter1Freq = Math.pow(document.getElementById('filter1Freq').value,3)/50;
	this.filter1FMAmount = document.getElementById('filter1FreqMod').value*this.filter1Freq/200;

	//FILTER CHARACTER

	if(keyOnOrOff[0]==true){
		this.filter1Freq = ((this.filter1Freq*(Math.log(this.noteFrequency)/Math.log(2)) + 1)/9);
	}
	...
	if(this.filter1Freq>20000){this.filter1Freq=20000};
	if(this.filter1Freq<50){this.filter1Freq=50};
	...
	this.filter1ResonanceValue = document.getElementById('filter1Resonance').value/(6 + this.subFilter1Multiplier);
	...
	if(filterTypes[0]=='lowpass'){this.filter1Gain.gain.value = 1};
	...
	this.lfoSpeed = Math.pow(document.getElementById('lfoSpeed').value,2)*this.lfoKeytrack/400;
	this.lfo.frequency.value = this.lfoSpeed;
	this.lfo.start();
	this.lfoGain = context.createGain();
	this.lfoGain.gain.value = Math.pow(document.getElementById('lfoLevel').value,2)/26000;
	...
	if(this.glide==true){this.filterMod.frequency.setTargetAtTime(this.newNoteFrequency/2, context.currentTime, this.glideAmount)};
	...
	if(this.filter1Freq<1000){
		this.filter1ResonanceValue = this.filter1ResonanceValue - (((1000-this.filter1Freq)/1000)*(document.getElementById('filter1FreqMod').value/100))
	}

	this.filter1FMAmount = (this.filter1FMAmount/2) +(this.filter1Env * this.filter1FMAmount/2);
	this.filter1.Q.value = this.filter1ResonanceValue;
	this.filter1FM.gain.value = this.filter1FMAmount;

	if(filterTypes[0]!='lowpass'){this.filter1FM.gain.value = 0};
	...
	this.filter1Peak = this.filter1Freq + (5000 + this.filter1Freq*1.5)*(1 - this.filter1Env);
	...
	this.filter1PeakFMAmount = this.filter1Peak*this.filter1FMAmount/this.filter1Freq;

	if(this.attackEnv<0.05){
		this.filter1.frequency.setValueAtTime(this.filter1Peak, context.currentTime);
	}else{
		this.filter1.frequency.setValueAtTime(this.filter1Freq, context.currentTime);
	}

	//ATTACK

	this.filter1SusLevel = this.filter1Freq + ((this.filter1Peak - this.filter1Freq)*this.sustainEnv*this.sustainEnv*4*(1 - this.filter1Env)); 
	this.filter1ResPeak = (this.filter1ResOrig - this.filter1ResonanceValue)*(1 - this.filter1Env) + this.filter1ResonanceValue;
	this.filter1ResSus = (this.filter1ResPeak - this.filter1ResonanceValue)*(this.filter1SusLevel/this.filter1Freq) + this.filter1ResonanceValue;

	this.osc1MasterGain.gain.setTargetAtTime(this.osc1Level, context.currentTime, this.attackEnv/3);
	this.osc2MasterGain.gain.setTargetAtTime(this.osc2Level, context.currentTime, this.attackEnv/3);
	this.whiteNoiseGain.gain.setTargetAtTime(this.whiteLevel, context.currentTime, this.attackEnv/3);
	this.filter1.frequency.setTargetAtTime(this.filter1Peak, context.currentTime, this.attackEnv/3);
	this.subGain.gain.setTargetAtTime(this.subLevel, context.currentTime, this.attackEnv/3);
	this.osc1MasterGain.gain.cancelScheduledValues(context.currentTime + this.attackEnv/3);
	this.osc2MasterGain.gain.cancelScheduledValues(context.currentTime + this.attackEnv/3);
	this.whiteNoiseGain.gain.cancelScheduledValues(context.currentTime + this.attackEnv/3);
	this.filter1.frequency.cancelScheduledValues(context.currentTime + this.attackEnv/3);
	this.lfofilter1.gain.setTargetAtTime(this.filter1Freq, context.currentTime, this.attackEnv/3);
	this.filter1.Q.setTargetAtTime(this.filter1ResPeak, context.currentTime, this.attackEnv/3);
	if(filterTypes[0]=='lowpass'){this.filter1FM.gain.setTargetAtTime(this.filter1PeakFMAmount, context.currentTime, this.attackEnv/3);};

	//DECAY

	this.osc1MasterGain.gain.setTargetAtTime(this.sustainEnv1, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	this.osc2MasterGain.gain.setTargetAtTime(this.sustainEnv2, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	this.whiteNoiseGain.gain.setTargetAtTime(this.whiteSustain, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	this.filter1.frequency.setTargetAtTime(this.filter1SusLevel, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	this.lfofilter1.gain.setTargetAtTime(this.filter1SusLevel, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	this.filter1.Q.setTargetAtTime(this.filter1ResSus, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	this.subGain.gain.setTargetAtTime(this.subSustain, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5);
	if(filterTypes[0]=='lowpass'){this.filter1FM.gain.setTargetAtTime(this.filter1FMAmount*this.filter1SusLevel/this.filter1Freq, context.currentTime + this.attackEnv/3 + 0.01, this.decayEnv/5)};
	...
	this.stop = (blah) => {
		...
		this.osc1MasterGain.gain.setTargetAtTime(0.000001, context.currentTime, this.release/4 + 0.002);
		this.osc2MasterGain.gain.setTargetAtTime(0.000001, context.currentTime, this.release/4 + 0.002);
		this.whiteNoiseGain.gain.setTargetAtTime(0.000001, context.currentTime, this.release/4);
		this.subGain.gain.setTargetAtTime(0.000001, context.currentTime, this.release/4 + 0.002);
		this.filter1.frequency.setTargetAtTime(this.filter1Freq, context.currentTime, this.filterRelease/3 + 0.1);
		this.lfofilter1.gain.setTargetAtTime(this.filter1Freq*this.filter1Env, context.currentTime, this.filterRelease/4);
		if(filterTypes[0]=='lowpass'){this.filter1FM.gain.setTargetAtTime(0, context.currentTime, this.filterRelease/4)};
		this.filter1.Q.setTargetAtTime(this.filter1ResonanceValue/2, context.currentTime, this.filterRelease/4);
		this.osc1MasterGain.gain.setTargetAtTime(0.000001, context.currentTime + this.release + 0.05, 0.1);
		this.osc2MasterGain.gain.setTargetAtTime(0.000001, context.currentTime + this.release + 0.05, 0.1);
		this.whiteNoiseGain.gain.setTargetAtTime(0.000001, context.currentTime + this.release + 0.05, 0.1);
		this.subGain.gain.setTargetAtTime(0.000001, context.currentTime + this.release + 0.05, 0.1);
		if(filterTypes[0]=='lowpass'){this.filter1FM.gain.setTargetAtTime(this.filter1FMAmount, context.currentTime, this.release/4 + 0.002)};

		whichNotesAreOnRelease.push(this);
		if(type!='sequencer'){this.x[note] = null}
		...
		if(octave==0.5*currentOctave){
			...
			if((g==0)||(g==8)||(g==1)||(g==9)||(g==2)){
				whichKeysAreLit[g]--;
				if(whichKeysAreLit[g]<0){whichKeysAreLit[g]=0};
				if(whichKeysAreLit[g]==0){
    				document.getElementById('note' + g).style.border = '1px solid #a0a0a0';
        	...
        	if(whichKeysAreLit[note]<0){whichKeysAreLit[note]=0};
        	if(whichKeysAreLit[note]==0){
            	document.getElementById('note' + note).style.border = '1px solid #a0a0a0';
    ...
    this.terminate = () => {
    	...
    	this.whiteNoiseGain.gain.cancelScheduledValues(context.currentTime);
    	this.subGain.gain.cancelScheduledValues(context.currentTime);
    	this.osc1MasterGain.gain.setTargetAtTime(0.000001, context.currentTime, 0.003);
    	this.osc2MasterGain.gain.setTargetAtTime(0.000001, context.currentTime, 0.003);
    	this.whiteNoiseGain.gain.setTargetAtTime(0.000001, context.currentTime, 0.003);
...
function stopNote(note,type){
	let x = null;
	if(type=='key'){x = keyArray}
		else if(type=='mouse'){x = mouseArray}
...
function octaveChange(oct){
	...
	if((x>0.2)&&(x<4.1)){4
		...
		if(currentOctave!=1){
			let y = '#2db5ff'
			let x;
			if(currentOctave>1){x = 'octUp'}else if(currentOctave<1){x = 'octDown'};
			if((currentOctave>2)||(currentOctave<0.5)){y = 'red'};
...
function lfoType(lfo,type){
	...
	let x = 'sine';
	if(type==1){x='sawtooth'}else if(type==2){x='triangle'}else if(type==3){x='square'};
	if(lfo==1){lfoType1=x}else{lfoType2=x};
...
function filterType(filter,type){
	document.getElementById('filter' + filter + 'lowpass').style.color = '#a8a8a8';
	document.getElementById('filter' + filter + 'highpass').style.color = '#a8a8a8';
	document.getElementById('filter' + filter + 'bandpass').style.color = '#a8a8a8';
	...
	if((document.hasFocus()==true)||(checkTouchDevice()==true)){
		...
		if(currentBeat==1){
			for(i in recordedNotes){
				if((quantize==true)&&(recordedNotes[i].beat==loopLength)&&(recordedNotes[i].start>0.875)&&(justPressedPlay==true)){
		...
		if(play==true){justPressedPlay = false};
...
for(i=startOnThis;i<=endOnThis;i++){
	...
	setTimeout(function(){
		...
		if((toggleInfo[0]==true)&&(play==true)&&((document.hasFocus()==true)||(checkTouchDevice()==true))){
			if(kickData[tt]==true){
				playSound(kick,0);
			}
			document.getElementById('kick'+tt).style.border = '2px solid #2db5ff';
			setTimeout(function(){document.getElementById('kick'+tt).style.border = '1px solid rgba(39, 31, 51, 0.3)';},speed/4);
		}

		if((toggleInfo[1]==true)&&(play==true)&&((document.hasFocus()==true)||(checkTouchDevice()==true))){
			if(clapData[tt]==true){
				playSound(clap,0);
				playSound(clap2,0);
			}
			document.getElementById('clap'+tt).style.border = '2px solid #2db5ff';
			setTimeout(function(){document.getElementById('clap'+tt).style.border = '1px solid rgba(39, 31, 51, 0.3)';},speed/4);
		}

		if((toggleInfo[2]==true)&&(play==true)&&((document.hasFocus()==true)||(checkTouchDevice()==true))){
			if(hatData[tt]==true){
				playSound(hat,0);
			}
			document.getElementById('hat'+tt).style.border = '2px solid #2db5ff';
			setTimeout(function(){document.getElementById('hat'+tt).style.border = '1px solid rgba(39, 31, 51, 0.3)';},speed/4);
		}
	},multiplier*speed/4)
}
...
if(counter%4==1){notx = true}
	...
	if(continueMetronome==true){let timeout3 = setTimeout(function(){metronomeFunction();clearTimeout(timeout3)},speed);}
	if(counter>=loopLength){counter=1}else{counter++;}
}
...
function pressClear(){
	...
	let timeout5 = setTimeout(function(){document.getElementById('clearButton').style.color = '#a8a8a8';clearTimeout(timeout5)},500);
...
function savePatch(){
    ...
    if(testtest==false){
     ...
     document.getElementById("saveButton").disabled = false;
	    ...
	    currentSound.filterEnvelopeAmount = document.getElementById("filter1Env").value;
...
window.onload = function(){
	...
	request.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			for(i in allPatches){
				selectData = selectData + '<option class="option">' + allPatches[i].patchName + '</option>';
			}
	...
	session.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if(sessionPatchData != null){
				...
				document.getElementById('echoLevel').value = parseInt(newPatchData.echoLevel);
				echoLevelUpdate(parseInt(newPatchData.echoLevel));
				document.getElementById('echoSpeed').value = parseInt(newPatchData.echoSpeed);
				echoSpeedUpdate(parseInt(newPatchData.echoSpeed));
				document.getElementById('filter1Freq').value = parseInt(newPatchData.filterCutoff);
				document.getElementById('filter1Env').value = parseInt(newPatchData.filterEnvelopeAmount);
				document.getElementById('filter1FreqMod').value = parseInt(newPatchData.filterFM);
				if(newPatchData.filterKeytrack==true){filterKey(1,'On');}
				filterType(1,newPatchData.filterType);
				document.getElementById('lfoLevel').value = parseInt(newPatchData.lfoLevel);
				for(i=0;i<=4;i++){
					if(newPatchData.lfoSend[i]==true){
						lfoSends(2,i);
					}
				}
				document.getElementById('lfoSpeed').value = parseInt(newPatchData.lfoSpeed);
				lfoType(2,parseInt(newPatchData.lfoWave));
				loopUpdate(newPatchData.loopLength/4);
				document.getElementById('masterGain').value = parseInt(newPatchData.masterGain);
				masterGainControl(newPatchData.masterGain)
				document.getElementById('noiseAmpMod').value = parseInt(newPatchData.noiseAM);
				document.getElementById('whiteLevel').value = parseInt(newPatchData.noiseLevel);
				document.getElementById('osc1Coarse').value = parseInt(newPatchData.osc1Coarse);
				document.getElementById('osc1FM').value = parseInt(newPatchData.osc1FM);
				document.getElementById('osc1Fine').value = parseInt(newPatchData.osc1Fine);
				document.getElementById('osc1Level').value = parseInt(newPatchData.osc1Level);
				document.getElementById('osc1PW').value = parseInt(newPatchData.osc1PW);
				oscWaveUpdate(1,newPatchData.osc1Wave);
				document.getElementById('osc2Coarse').value = parseInt(newPatchData.osc2Coarse);
				document.getElementById('osc2FM').value = parseInt(newPatchData.osc2FM);
				document.getElementById('osc2Fine').value = parseInt(newPatchData.osc2Fine);
				document.getElementById('osc2Level').value = parseInt(newPatchData.osc2Level);
				document.getElementById('osc2PW').value = parseInt(newPatchData.osc2PW);
				oscWaveUpdate(2,newPatchData.osc2Wave);
				document.getElementById('distortion').value = parseInt(newPatchData.overdrive);
				distortionValue(parseInt(newPatchData.overdrive));
				document.getElementById('release').value = parseInt(newPatchData.release);
				if(newPatchData.quantize==true){quantizeOn()};
				document.getElementById('glideAmount').value = parseInt(newPatchData.portamento);
				document.getElementById('filter1Resonance').value = parseInt(newPatchData.resonance);
				speed = newPatchData.speed;
				document.getElementById('playbackSpeed').value = 100 - (newPatchData.speed - 250)/5;
				document.getElementById('subLevel').value = parseInt(newPatchData.subLevel);
				document.getElementById('sustain').value = parseInt(newPatchData.sustain);
				document.getElementById('tremoloLevel').value = parseInt(newPatchData.tremoloLevel);
				tremoloLevel(parseInt(newPatchData.tremoloLevel));
				tremoloSpeed(parseInt(newPatchData.tremoloSpeed));
				document.getElementById('tremoloSpeed').value = parseInt(newPatchData.tremoloSpeed);
				...
				if(newPatchData.kickData!=null){
					...
					for(i=0;i<=15;i++){
						if(kickData[i]==true){
							document.getElementById('kick' + i).style.background = 'rgba(39, 31, 51, 0.8)';
						}
						if(clapData[i]==true){
							document.getElementById('clap' + i).style.background = 'rgba(39, 31, 51, 0.8)';
						}
						if(hatData[i]==true){
							document.getElementById('hat' + i).style.background = 'rgba(39, 31, 51, 0.8)';
						}
					}
					for(i=0;i<=2;i++){
				...
				setTimeout(function(){document.getElementById('select').value = sessionPatchName},1000);
			}
		}
	}	
}
...
function drumSequence(sound,number){
	...
	if(x[number]==false){
		x[number] = true;
		document.getElementById(sound + number).style.background = 'rgba(39, 31, 51, 0.8)';
	}else{
		x[number] = false;
		document.getElementById(sound + number).style.background = 'rgba(39, 31, 51, 0.2)';
	}
}
